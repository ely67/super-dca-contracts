    ...
    AutomateTaskCreator
{
    ...
    function _createTask() internal {
        ...
        moduleData.args[1] = _timeTriggerModuleArg(uint128(block.timestamp), uint128(60000));
    ...
    function _getUnderlyingToken(
        ...
        if (address(_token) == address(wethx)) {
            return address(weth);
        }
    ...
    function getTradeInfo(address _trader, uint _tradeIndex) public view returns (
        SuperDCATrade.Trade memory trade
    ){
    ...
    function getLatestTrade(address _trader) public view returns (
        ...
        if (dcaTrade.tradeCountsByUser(_trader) > 0) {
            ...
            trade = SuperDCATrade.Trade(0, 0, 0, 0, 0, 0, 0, 0);
        }
    }
